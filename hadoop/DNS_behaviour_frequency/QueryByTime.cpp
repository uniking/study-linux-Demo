...
int get_hostname_by_sip_day(map<string, long>& hostnameSet, const char* sip, const char* day)
{//select hostname from dns where DATE(time) = DATE("2016-08-11") and sip="192.168.220.10";
	int t,r;
	char query[1024];
	snprintf(query, 1024, "select hostname from %s where DATE(time) = DATE(\"%s\") and sip=\"%s\"", TABLE_NAME, day, sip);
	...
	while(row = mysql_fetch_row(res))
	{
		for(t=0; t<mysql_num_fields(res); t++)
		{
			map<string, long>::iterator pM = hostnameSet.find(string(row[t]));
			if( pM == hostnameSet.end() )
				hostnameSet.insert(make_pair(string(row[t]), 1));
			else
				pM->second++;
		}
	}
...
int database_to_item(string site, int e, map<string, list<DATA_ITEM> >& Matrix, list<string>& ignoreDay,bool filtrate)
{
	...
	{		
		...
		while(SipIter != sipSet.end())
		{
			...
			map<string, long> hostnameSet;
			get_hostname_by_sip_day(hostnameSet, (*SipIter).c_str(), (*DayIter).c_str());
			map<string, long>::iterator HostnameIter = hostnameSet.begin();
			while(HostnameIter != hostnameSet.end())
			{
				if(filtrate)
				{
					if(!hostname_filtrate(HostnameIter->first))
					{
						dIt.hostname.insert(*HostnameIter);
					}
				}
				else
					dIt.hostname.insert(*HostnameIter);
