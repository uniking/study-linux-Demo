...
public class UserCommandStatistics implements Serializable {
    ...
    public UserCommandStatistics(Map<String, Object> rawMap){
        if(rawMap.get(COMMAND_NAME)!=null)
            this.commandName = String.valueOf(rawMap.get(COMMAND_NAME));
        if(rawMap.get(MEAN)!=null)
            this.mean = Double.parseDouble(rawMap.get(MEAN).toString());
        if(rawMap.get(STDDEV)!=null)
            this.stddev = Double.parseDouble(rawMap.get(STDDEV).toString());
        if(rawMap.get(IS_LOW_VARIANT)!=null)
            this.isLowVariant = Boolean.parseBoolean(rawMap.get(IS_LOW_VARIANT).toString());
    }
