              ...
              getdns_transaction_t   transaction_id)
{
	...
	printf( "Callback for query \"%s\" with request ID %d.\n"
	      , (char *)userarg, transaction_id );

	switch(callback_type) {
	case GETDNS_CALLBACK_CANCEL:
		printf("Transaction with ID %d was cancelled.\n", transaction_id);
		return;
	case GETDNS_CALLBACK_TIMEOUT:
		printf("Transaction with ID %d timed out.\n", transaction_id);
		return;
	case GETDNS_CALLBACK_ERROR:
		printf("An error occurred for transaction ID %d.\n", transaction_id);
		return;
	default: break;
	}
	...
	if ((r = getdns_dict_get_int(response, "status", &status)))
		...
		fprintf(stderr, "The search had no results, and a return value of %d.\n", status);
