...
#if (LINUX_VERSION_CODE < KERNEL_VERSION(3,10,0))
#define STR_PRINTF(str, args...) sprintf(page + len, str, ## args);
#else
#define STR_PRINTF(str, args...) seq_printf(m, str, ## args);
#endif
...
#endif
{
 ...
 {
  case 0:
   len += STR_PRINTF("time = %ld jiffies\n", act_jiffies);
   break;
  case 1:
   len += STR_PRINTF("time = %d msecs\n", val);
   break;
  case 2:
  ...
  case 3:
   ...
   break;
  default:
   len += STR_PRINTF("<not implemented>\n");
   break;
 }
...
#else
static ssize_t time_write(struct file *file, const char __user *buffer, size_t count,
 loff_t *off)
#endif
{
 char kbuf[2];
